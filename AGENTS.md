...
- Read `PLANS.md` only when creating or implementing an ExecPlan for a complex feature or significant refactor.
- For prediction work, use the relevant setup/tests in `README.md` (`pytest tests/ -v`) and inspect the affected workflow under `.github/workflows/` before running it. Scheduled/manual prediction workflows are not documentation checks; preserve their state and financial/data boundaries.

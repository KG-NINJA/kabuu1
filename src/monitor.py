# -*- coding: utf-8 -*-
"""予測メトリクスを可視化する Flask + Plotly ベースのダッシュボード。"""
...
def ensure_template() -> None:
    ...
    dashboard_lines = [
        ...
        "                const errors = metrics.map(",
        "                    m => (m.prediction_error || 0) * 100",
        "                );",
        "",
        ...
        "                const metricsHtml = recentMetrics",
        "                    .map(m => `",
        '                    <div class=\\"metric-card\\">',
        "                        <h3>${new Date(m.timestamp)",
        "                            .toLocaleString('ja-JP')}</h3>",
        "                        <p>銘柄: ${m.ticker || '不明'}</p>",
        "                        <p>予測価格: ${m.predicted_price",
        "                            ? m.predicted_price.toFixed(2) : 'N/A'}</p>",
        "                        <p>実際の価格: ${m.actual_price",
        "                            ? m.actual_price.toFixed(2) : '未反映'}</p>",
        "                        <p>誤差: ${m.prediction_error",
        "                            ? (m.prediction_error * 100).toFixed(2) : 'N/A'}%</p>",
